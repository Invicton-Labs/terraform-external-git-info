locals {
  ...
  commands_common = [
    local.var_pull ? [null, "git pull"] : (local.var_fetch ? [null, "git fetch"] : null),
    local.var_get_current_branch ? ["current_branch", "git rev-parse --abbrev-ref HEAD"] : null,
    local.var_get_local_branches ? ["local_branches", "git branch --format='%(refname:short)' --no-color"] : null,
    local.var_get_remote_branches ? ["remote_branches", "git branch --no-color --remote"] : null,
    local.var_get_commit_hash ? ["commit_hash", "git rev-parse HEAD"] : null,
    local.var_get_current_tags ? ["current_tags", "git tag --points-at HEAD"] : null,
    local.var_get_remotes ? ["remotes", "git remote -v"] : null,
  ]

  commands_unix = [
    local.var_get_tags ? ["tags", "git show-ref --tags || (exitcode=$?; if [ $exitcode != 1 ]; then exit $exitcode; fi;)"] : null,
  ]

  commands_windows = [
    local.var_get_tags ? ["tags", "$output = git show-ref --tags; if (($LASTEXITCODE -ne 0) -and ($LASTEXITCODE -ne 1)) { $ErrorActionPreference = \"Continue\"; Exit $LASTEXITCODE }; Write-Output $output; $ErrorActionPreference = \"Stop\";"] : null,
  ]
...
module "shell_script" {
  ...
  version                   = "~> 0.3.1"
  working_dir               = local.var_working_dir != null ? local.var_working_dir : path.root
